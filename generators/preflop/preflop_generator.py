...
class PreflopProfileGenerator(BaseProfileGenerator):
    ...
    def _generate_header(self, settings):
        ...
        header += f"// - Early Position Range: {settings['ep_range']}% (Threshold: {self.get_handrank_threshold(settings['ep_range'])})\n"
        header += f"// - Middle Position Range: {settings['mp_range']}% (Threshold: {self.get_handrank_threshold(settings['mp_range'])})\n"
        header += f"// - Late Position Range: {settings['lp_range']}% (Threshold: {self.get_handrank_threshold(settings['lp_range'])})\n"
